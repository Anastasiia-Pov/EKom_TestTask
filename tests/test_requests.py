...
# невалидные шаблоны
form_0_1 = {"nonexisting_field": "1997.19.65",  # такого поля нет и неверная дата (определится как текст)
...
form_0_4 = {"birthday": "1997.14.09"} # такое поле есть, но дата не соответствует формату

form_0_5 = {"day": "1997-10-09"} # поля нет, но дата соответствует формату
...
async def test_form_0_1(test_client: AsyncClient):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data == {"nonexisting_field": "text",
                    "field_1": "phone"
                    }
